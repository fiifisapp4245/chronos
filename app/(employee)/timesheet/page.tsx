...
export default function TimesheetPage() {
  ...
  React.useEffect(() => {
    ...
    getMyTimesheet(from, to).then((data) => {
      if (!cancelled) {
  ...
  React.useEffect(() => {
    ...
    getMyTimesheet(clampedFrom, clampedTo).then((data) => {
      if (!cancelled) {
  ...
  const visibleDays = onlyAdjusted
    ? periodDays.filter((d) => d.adjustments.length > 0)
    : periodDays
